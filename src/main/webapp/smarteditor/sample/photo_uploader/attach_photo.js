...
//변수 선언 및 초기화
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){				console.log("callAjax////"+sUploadURL+"/////"+tempFile);
    	var oAjax = jindo.$Ajax(sUploadURL, {
			type: 'xhr',
			method : "post",
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
				var sResString = res._response.responseText;				console.log("Ajax 요청 완료");
				if (res.readyState() == 4) {
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : 'file_uploder.jsp',	//샘플 URL입니다.
